...
int32_t iot_i2c_read_async( IotI2CHandle_t const pxI2CPeripheral, uint8_t *const pvBuffer, size_t xBytes)
{
    ...
    */
    if (xTimerPendFunctionCall(read_async_cb, (void *)iot_i2c_handler, 0, FREERTOS_TIMER_WAIT_TIME) != pdTRUE) {
...
int32_t iot_i2c_write_async( IotI2CHandle_t const pxI2CPeripheral, uint8_t *const pvBuffer, size_t xBytes)
{
    ...
    /* FREERTOS_TIMER_WAIT_TIME is the amount of time the calling task should remain
       ...
       available on the timer queue if the queue is found to be full
    */
    if (xTimerPendFunctionCall(write_async_cb, (void *)iot_i2c_handler, 0, FREERTOS_TIMER_WAIT_TIME) != pdTRUE) {
