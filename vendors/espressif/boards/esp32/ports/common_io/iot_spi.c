...
int32_t iot_spi_write_async(IotSPIHandle_t const pxSPIPeripheral, uint8_t * const pvBuffer, size_t xBytes)
{
    ...
    */
    if (xTimerPendFunctionCall(spi_write_async_cb, (void *)spi_ctx, 0, FREERTOS_TIMER_WAIT_TIME) != pdTRUE) {
...
int32_t iot_spi_read_async(IotSPIHandle_t const pxSPIPeripheral, uint8_t * const pvBuffer, size_t xBytes)
{
    ...
    /* FREERTOS_TIMER_WAIT_TIME is the amount of time the calling task should remain
       ...
       available on the timer queue if the queue is found to be full
    */
    if (xTimerPendFunctionCall(spi_read_async_cb, (void *)spi_ctx, 0, FREERTOS_TIMER_WAIT_TIME) != pdTRUE) {
...
int32_t iot_spi_transfer_async(IotSPIHandle_t const pxSPIPeripheral, uint8_t * const pvTxBuffer, uint8_t * const pvRxBuffer, size_t xBytes)
{
    ...
    /* FREERTOS_TIMER_WAIT_TIME is the amount of time the calling task should remain
       ...
       available on the timer queue if the queue is found to be full
    */
    if (xTimerPendFunctionCall(spi_read_write_transfer_cb, (void *)spi_ctx, 0, FREERTOS_TIMER_WAIT_TIME) != pdTRUE) {
